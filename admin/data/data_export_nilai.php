...
$identitasUjian = mysqli_fetch_array(mysqli_query($con,"select * from tbl_ujian where id_ujian='".$_POST['idUjian']."'"));
header("Content-Disposition: attachment; filename=Daftar_Nilai_".$identitasUjian['kode_ujian'].".xls")
?>
...
<body>
	...
	<table width="100%" border="1" cellpadding="2" class="table">
		...
		while($resStatus = mysqli_fetch_array($dataStatus)){
			...
			$siswa = mysqli_fetch_array(mysqli_query($con,"select * from tbl_siswa where nopes_siswa='".$resStatus['nopes_siswa']."'"));
